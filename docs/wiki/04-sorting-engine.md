...
- `deriveMergeFramesFromHistory(initialValues, history)`: Transforma a lista de eventos brutos de histórico em uma sequência canônica de `MergeVisualStepFrame` para consumo pela UI e pelo futuro Replay. Como cada evento já possui os snapshots imutáveis pré-gravados, a derivação dos quadros percorre sequencialmente os $m$ eventos do histórico ($m \in O(n \log n)$), com complexidade temporal $O(m)$ quando o trabalho por evento é constante (reaproveitando snapshots já formados). Na gravação original pela engine pura, cada emissão de evento realiza cópia rasa dos arrays de comprimento $n$, acumulando trabalho de $O(m \cdot n)$ cópias de ponteiros ao longo da sessão inteira;
- **Acesso a quadros preparados vs. Derivação a frio:** A indexação direta sobre uma lista de quadros previamente derivada (`frames[stepIndex]`) é rigorosamente $O(1)$ imediata em tempo. Já a função utilitária `reconstructMergeStepAt(initialValues, history, stepIndex)` reexecuta a derivação sequencial internamente quando chamada de forma avulsa (custo $O(m)$), não devendo ser invocada repetidamente em laços sem reaproveitar a lista derivada;
...
- **Armazenamento de Snapshots na Sessão:** O histórico grava $m \in O(n \log n)$ eventos imutáveis (`MergeStepRecord[]`), cada qual retendo uma cópia rasa do array `valuesSnapshot` ($n$ referências) e, durante a intercalação ativa, `bufferSnapshot` (até $n$ referências). Como todas as instâncias de `MergeElement` são imutáveis e compartilhadas por ponteiro (zero clonagem profunda de dados), o histórico em si retém estritamente ponteiros aos elementos existentes. Contabilizando referências nos vetores principais e buffers:
  ...
  Essa contagem delimitada de referências aplica-se estritamente aos snapshots de elementos da engine; o custo de memória global da aplicação inclui adicionalmente nós do React, closures de eventos e reconciliação da árvore virtual, dispensando recalculo algorítmico na engine pura.
