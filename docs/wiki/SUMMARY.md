...
| **Testes** | **Ativos para Engine, FSM, Campanha, Sessão, Pontuação, Replay, Pseudocódigo, Early Exit, Persistência, Geração Procedural, Briefings e Selection Sort.** Vitest (`vitest ^5.0.0`) instalado e operacional; **268 testes automatizados passando 100% verde** em 16 arquivos de teste: `bubbleSortEngine.test.ts` (39), `bubbleSortFsm.test.ts` (3), `campaignSummary.test.ts` (4), `sessionMetrics.test.ts` (5), `replayModel.test.ts` (11), `replayPseudocode.test.ts` (12), `protocolScore.test.ts` (11), `persistence.test.ts` (60), `arrayGenerator.test.ts` (31), `briefing.test.ts` (16), `selectionSortEngine.test.ts` (18), `selectionConstraints.test.ts` (16), `selectionTutorialGuide.test.ts` (3), `selectionCampaign.test.ts` (19), `selectionReplayModel.test.ts` (9) e `selectionReplayPseudocode.test.ts` (9). Testes de interface React e E2E permanecem planejados. |
...
Os marcos **P0** (fundação algorítmica da esteira de triagem do Bubble Sort e encerramento da campanha), as etapas **P1.1 a P1.10** e o milestone **P2.1 (Selection Sort)** foram integralmente concluídos:
...
2. **Mapeamento Comparativo Multi-Algoritmo (P3.2):** Execução do mesmo vetor gerado por seed entre Bubble e Selection Sort.
