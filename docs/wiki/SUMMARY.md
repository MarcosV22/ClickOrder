...
| **Telas Existentes** | 11 telas ativas: `HomeScreen` (seleção de modo, briefing e tutorial), `ProtocolModeBriefingScreen` (briefing intermediário orientado a dados com objetivos, passos operacionais, particularidades, destaques de telemetria e geração tardia), `TutorialScreen` (interativa Bubble P1.1), `SelectionTutorialScreen` (interativa Selection P2.1-C sobre vetor [4, 1, 3]), `GameScreen` (Bubble com timer monotônico e suporte a `CANONICAL`/`EARLY_EXIT`), `SelectionGameScreen` (Selection Sort com 3 fases procedurais, scanner sem movimentação e commit com transferência de longa distância — P2.1-D), `ResultScreen` (com destaque da Pontuação do Protocolo, métricas de comparações evitadas no desafio, suporte a Selection com pseudocódigo canônico de 13 instruções e botão de replay ativo), `ReplayScreen` (reprodução passo a passo Bubble), `SelectionReplayScreen` (reprodução passo a passo Selection com pseudocódigo sincronizado — P2.1-E), `CampaignCompleteScreen` (homologação Bubble) e `SelectionCampaignCompleteScreen` (homologação Selection Sort — P2.1-D). |
| **Backend** | **Inexistente.** Não há servidor de aplicação, microserviços, GraphQL ou endpoints REST. |
| **Persistência** | **Implementada e Desacoplada Multi-Protocolo (P1.6, P1.7 e P2.1-F / ADR 0006, ADR 0007 e ADR 0015).** Armazenamento local via `localStorage` (chave canônica `sorting_station_v1_save`, **Schema v3**) gerenciado pelo módulo puro `src/game/persistence/`. Armazena progresso e recordes isolados por protocolo (`protocols.bubble` e `protocols.selection`), com pipeline de migração transparente e testável ($v1 \rightarrow v2 \rightarrow v3$ e $v2 \rightarrow v3$), desempate estritamente por menor número de erros (tempo descritivo nunca desempata) e fallback gracioso em memória. O Selection Sort possui persistência de tutorial resistente a F5, desbloqueio de fases (1..3) e recordes factuais de pontuação. Iniciar nova campanha sempre inicia na Fase 1. Quadros de Replay (`StepRecord[]`, `SelectionStepRecord[]`, `frames`) permanecem 100% voláteis em memória RAM sem inflar o storage. |
| **Testes** | **Ativos para Engine, FSM, Campanha, Sessão, Pontuação, Replay, Pseudocódigo, Early Exit, Persistência, Geração Procedural, Briefings, Selection Sort e Configuração Data-Driven.** Vitest (`vitest ^5.0.0`) instalado e operacional; **271 testes automatizados passando 100% verde** em 17 arquivos de teste: `bubbleSortEngine.test.ts` (39), `bubbleSortFsm.test.ts` (3), `campaignSummary.test.ts` (4), `sessionMetrics.test.ts` (5), `replayModel.test.ts` (11), `replayPseudocode.test.ts` (12), `protocolScore.test.ts` (11), `persistence.test.ts` (60), `arrayGenerator.test.ts` (31), `briefing.test.ts` (16), `selectionSortEngine.test.ts` (18), `selectionConstraints.test.ts` (16), `selectionTutorialGuide.test.ts` (3), `selectionCampaign.test.ts` (19), `selectionReplayModel.test.ts` (9), `selectionReplayPseudocode.test.ts` (9) e `campaignCompleteConfig.test.ts` (3). Testes de interface React e E2E permanecem planejados. |
| **Outros Algoritmos** | Selection Sort possui **ciclo completo de implementação concluído: domínio puro, FSM bimodal, geração procedural (`selectionConstraints`), briefing canônico, tutorial interativo guiado, campanha jogável de 3 fases procedurais, Replay somente-leitura com pseudocódigo sincronizado de 13 instruções e persistência multi-protocolo Schema v3 (Milestone P2.1 concluído via ADRs 0011, 0012, 0013, 0014 e 0015)**. Insertion Sort, Merge Sort e Quick Sort são futuros. |

---

## 3. Prioridade Atual

Os marcos **P0** (fundação algorítmica da esteira de triagem do Bubble Sort e encerramento da campanha), as etapas **P1.1 a P1.10**, o milestone **P2.1 (Selection Sort)** e os subitens **P2.1-G-A** e **P2.1-G-B** foram integralmente concluídos:
...
1. **P2.1-G-C (Hub Simétrico de Protocolos na Home Screen):** Cards simétricos para Bubble, Selection e Insertion [Em Breve] com atalhos diretos para campanha, tutorial e demonstração;
2. **P2.1-G-D (Institucionalização do Modo Demonstração):** Conectar autoplay autônomo da engine ao Replay nos fluxos da Home e Briefing;
3. **Módulo Insertion Sort: Mecânica de Desvio e Encaixe de Cargas (P2.2):** Engine pura, FSM de elevação e deslocamento regressivo de elementos maiores.
