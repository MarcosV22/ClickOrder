...
| :--- | :--- |
| **Frontend** | React 19 (`^19.0.0`), TypeScript 5.7 strict, Vite 8 (`^8.2.2`), Tailwind CSS v4 inline (`@tailwindcss/vite`), Figma Make; idioma canônico pt-BR e proteção contra tradução automática (`translate="no"` / `notranslate`). |
| **Gameplay** | Bubble Sort didático sequencial estrito integrado à **Bubble Sort Engine pura** em `GameScreen.tsx` (**Milestones P0 e P1.1 a P1.10 concluídos**); botões de decisão pedagógica `[⇄ TROCAR]` e `[= MANTER]`, bloqueio de ações fora de sequência, animação simétrica de swap, fixação determinística (`OK`), cálculo de progresso real; esteira contínua com rolagem horizontal controlada sem quebra em viewports estreitos (`overflow-x-auto min-w-max`); 3 fases progressivas (vetores procedurais determinísticos de 4, 5 e 6 elementos via `src/game/generation/` / ADR 0009); telemetria factual descritiva de erros e dicas; eliminação da heurística arbitrária de eficiência (ADR 0003); **Replay da Execução somente-leitura** com navegação passo a passo, autoplay auto-stop e Quadro 0 inicial derivado deterministicamente de `history` (ADR 0004); **Pseudocódigo Sincronizado no Replay** com modelos canônico e de término antecipado (ADR 0005 e ADR 0008); **Homologação e UX Polish** transversal (P1.5); **Pontuação do Protocolo e Tempo Descritivo** (P1.7 / ADR 0007); **Modo Desafio / Variante Early Exit** (P1.8 / ADR 0008) com motor unificado; **Infraestrutura Global de Geração Procedural de Vetores** (P1.9 / ADR 0009); **Briefing dos Modos de Jogo** (P1.10 / ADR 0010); **Campanha Jogável do Selection Sort de 3 Fases** (P2.1-D / ADR 0013) com tela dedicada (`SelectionGameScreen`), animação de longa distância com translação proporcional, feedback formativo não punitivo, FSM bimodal estrita, conclusão própria (`SelectionCampaignCompleteScreen`) e isolamento em memória sem mutação do Schema v2; e **Replay Dedicado e Pseudocódigo Sincronizado do Selection Sort** (P2.1-E / ADR 0014) derivado puramente de `SelectionStepRecord[]` sem reexecução do algoritmo, sem frames fantasmas, 13 instruções estruturadas em português e habilitação de `[ ▶ VER EXECUÇÃO ]` na `ResultScreen`. |
| **Telas Existentes** | 12 telas ativas: `HomeScreen` (Hub Educacional Simétrico de Protocolos com cartões data-driven para Bubble Sort, Selection Sort e placeholder curricular de Insertion Sort, telemetria de fases/tutorial/recordes e atalhos para Briefing, Tutorial voluntário, Modo Desafio e Demonstração — P2.1-G-C), `ProtocolModeBriefingScreen` (briefing intermediário orientado a dados com objetivos, passos operacionais, particularidades, destaques de telemetria e ação secundária de ver demonstração), `TutorialScreen` (interativa Bubble P1.1 / harmonizada P2.1-G-B), `SelectionTutorialScreen` (interativa Selection P2.1-C sobre vetor [4, 1, 3]), `GameScreen` (Bubble com timer monotônico, banner relacional, barra de progresso contínua e suporte a `CANONICAL`/`EARLY_EXIT` — P2.1-G-B), `SelectionGameScreen` (Selection Sort com 3 fases procedurais, scanner sem movimentação e commit com transferência de longa distância — P2.1-D), `ResultScreen` (com destaque da Pontuação do Protocolo, métricas de comparações evitadas no desafio, suporte a Selection com pseudocódigo canônico de 13 instruções e botão de replay ativo), `ReplayScreen` (reprodução passo a passo Bubble com suporte a velocidade variável e modo demonstração), `SelectionReplayScreen` (reprodução passo a passo Selection com pseudocódigo sincronizado, suporte a velocidade variável e modo demonstração — P2.1-E/P2.1-G-D), `DemonstrationScreen` (orquestrador puro do Modo Demonstração Educacional canônico para Bubble e Selection — P2.1-G-D / ADR 0017), `CampaignCompleteScreen` (homologação Bubble data-driven — P2.1-G-B) e `SelectionCampaignCompleteScreen` (homologação Selection Sort wrapper — P2.1-G-B). |
| **Backend** | **Inexistente.** Não há servidor de aplicação, microserviços, GraphQL ou endpoints REST. |
| **Persistência** | **Implementada e Desacoplada Multi-Protocolo (P1.6, P1.7 e P2.1-F / ADR 0006, ADR 0007 e ADR 0015).** Armazenamento local via `localStorage` (chave canônica `sorting_station_v1_save`, **Schema v3**) gerenciado pelo módulo puro `src/game/persistence/`. Armazena progresso e recordes isolados por protocolo (`protocols.bubble` e `protocols.selection`), com pipeline de migração transparente e testável ($v1 \rightarrow v2 \rightarrow v3$ e $v2 \rightarrow v3$), desempate estritamente por menor número de erros (tempo descritivo nunca desempata) e fallback gracioso em memória. O Selection Sort possui persistência de tutorial resistente a F5, desbloqueio de fases (1..3) e recordes factuais de pontuação. Iniciar nova campanha sempre inicia na Fase 1. Quadros de Replay e execuções de demonstração permanecem 100% voláteis em memória RAM sem inflar nem alterar o storage. |
| **Testes** | **Ativos para Engine, FSM, Campanha, Sessão, Pontuação, Replay, Pseudocódigo, Early Exit, Persistência, Geração Procedural, Briefings, Selection Sort, Hub de Protocolos, Configurações Data-Driven e Modo Demonstração.** Vitest (`vitest ^5.0.0`) instalado e operacional; **303 testes automatizados passando 100% verde** em 19 arquivos de teste: `bubbleSortEngine.test.ts` (39), `bubbleSortFsm.test.ts` (3), `campaignSummary.test.ts` (4), `sessionMetrics.test.ts` (5), `replayModel.test.ts` (11), `replayPseudocode.test.ts` (12), `protocolScore.test.ts` (11), `persistence.test.ts` (60), `arrayGenerator.test.ts` (31), `briefing.test.ts` (16), `selectionSortEngine.test.ts` (18), `selectionConstraints.test.ts` (16), `selectionTutorialGuide.test.ts` (3), `selectionCampaign.test.ts` (19), `selectionReplayModel.test.ts` (9), `selectionReplayPseudocode.test.ts` (9), `campaignCompleteConfig.test.ts` (3), `protocolCatalog.test.ts` (15) e `demonstration.test.ts` (17). Testes de interface React e E2E permanecem planejados. |
| **Outros Algoritmos** | Selection Sort possui **ciclo completo de implementação concluído: domínio puro, FSM bimodal, geração procedural (`selectionConstraints`), briefing canônico, modo demonstração autônomo, tutorial interativo guiado, campanha jogável de 3 fases procedurais, Replay somente-leitura com pseudocódigo sincronizado de 13 instruções e persistência multi-protocolo Schema v3 (Milestone P2.1 concluído via ADRs 0011, 0012, 0013, 0014, 0015 e 0017)**. Insertion Sort possui registro curricular no catálogo de protocolos com status EM BREVE (P2.1-G-C). |

---

## 3. Prioridade Atual

Os marcos **P0** (fundação algorítmica da esteira de triagem do Bubble Sort e encerramento da campanha), as etapas **P1.1 a P1.10**, o milestone **P2.1 (Selection Sort)** e o macro-marco **P2.1-G (Auditoria, Padronização e Modo Demonstração)** foram integralmente concluídos e implementados:
...
- **P2.1-D Concluído (Gameplay e Campanha de 3 Fases do Selection Sort):** Tela própria `SelectionGameScreen.tsx` consumindo `SelectionSortEngine` imutável como única fonte de verdade; botoeira contextual com bloqueio mútuo e trava síncrona; comparação textual explícita ($A[j] < A[minIndex]$); feedback formativo não punitivo; transferência de longa distância com deslocamento horizontal proporcional e elevação vertical (`--swap-distance`); tela dedicada de conclusão `SelectionCampaignCompleteScreen.tsx`; união discriminada de resultados (`GameResult`); adaptação de `ResultScreen.tsx`; persistência puramente em memória da sessão sem alteração do Schema v2 do Bubble Sort; e 19 novos testes automatizados Vitest (totalizando 229 testes) — ADR 0013;
- **Hotfix Concluído (Responsividade Desktop e Overflow Vertical das Telas Longas):** Correção estrutural em contêineres de scroll (remoção de `justify-center` e `my-auto` em `ProtocolModeBriefingScreen`, `SelectionTutorialScreen`, `TutorialScreen`, `HomeScreen`, `ResultScreen`, `CampaignCompleteScreen`, `SelectionCampaignCompleteScreen`, `SelectionGameScreen`, `ReplayScreen` e `GameScreen`), adoção sistemática de `justify-start` com `pt-6 sm:pt-8` e `pb-16 sm:pb-24`, densidade visual calibrada e garantia de visibilidade total dos CTAs e botões de retorno em 1366x768, 1600x900 e 1920x1080;
- **P2.1-E Concluído (Replay Somente-Leitura e Pseudocódigo Sincronizado do Selection Sort):** Modelo puro de derivação de quadros a partir de `SelectionStepRecord[]` sem reexecução do algoritmo e sem frames fantasmas (`buildSelectionReplayFrames`); distinção entre inspeção ($i$, $j$, $minIndex$, novo mínimo vs manter) e commit (troca real vs consolidação sem troca); pseudocódigo canônico de 13 instruções em português (`SELECTION_SORT_PSEUDOCODE`) com destaque semântico de `INIT_MIN`, `IF_CONDITION`, `UPDATE_MIN`, `CHECK_SWAP` e `SWAP_STATEMENT`; componente `SelectionSortPseudocodePanel.tsx`; tela de replay dedicada `SelectionReplayScreen.tsx` com esteira semântica de `NumberedBox`, autoplay com auto-stop e responsividade desktop; ativação de `[ ▶ VER EXECUÇÃO ]` em `ResultScreen.tsx` e preservação estrita somente-leitura em `App.tsx` (ADR 0014), com 246 testes Vitest aprovados (+17 novos testes unitários puros);
- **P2.1-F Concluído (Persistência Multi-Protocolo Schema v3, Migração Transparente e Conclusão de P2.1):** Evolução para o Schema v3 (`protocols: { bubble, selection }`) sem IDs mágicos ou números artificiais; pipeline de migração determinístico $v1 \rightarrow v2 \rightarrow v3$ e $v2 \rightarrow v3$ preservando a chave canônica `sorting_station_v1_save`; persistência do Selection Sort (tutorial resistente a F5, desbloqueio de fases 1..3, histórico de conclusões e recordes atualizados pela regra de precisão canônica); garantia de início de campanha na Fase 1 com novas entradas procedurais; replay mantido 100% volátil em memória RAM; QA completo com validação de replay (`minIndexBefore` em `INSPECTION`) e regressão do Bubble Sort (ADR 0015), totalizando 268 testes Vitest aprovados (+22 novos testes de persistência e Replay QA);
- **P2.1-G-A Concluído (Auditoria e Padronização Educacional do Sorting Station):** Formalização do Charter Educacional oficial (ADR 0016), congelamento diegético da metáfora logística, consolidação do ciclo pedagógico de 6 camadas por protocolo (Briefing $\rightarrow$ Demonstração $\rightarrow$ Tutorial $\rightarrow$ Gameplay $\rightarrow$ Resultado $\rightarrow$ Replay/Reflexão), unificação da política não punitiva de score ($100 - 10 \times \text{erros} - 5 \times \text{dicas}$) e tempo descritivo com peso zero, especificação conceitual do padrão das 8 telas, congelamento do Glossário Canônico de termos, auditoria detalhada de inconsistências entre Bubble e Selection Sort, institucionalização do Modo Demonstração reutilizando componentes existentes de Replay e pseudocódigo sem duplicação de arquitetura, e definição do plano incremental de refatoração (`P2.1-G-B` a `P2.1-G-D`);
- **P2.1-G-B Concluído (Harmonização Visual, Estrutural e Pedagógica das Telas):**
  - Correções conceituais canônicas na Wiki e ADR 0016: Score denominado oficialmente "Pontuação do Protocolo", Permuta/Troca definida transversalmente como troca física entre duas posições (adjacente no Bubble, de longa distância no Selection) e consolidação da distinção entre fixação definitiva (`OK`/`FIXO`) e região ordenada provisória (`ORD`);
  - `NumberedBox` estendido com suporte ao papel semântico `ordered` (`ORD`) com borda esmeralda tracejada;
  - `TutorialScreen` do Bubble Sort harmonizado com a barra superior canônica (`[◀ VOLTAR]`, `[TUTORIAL INTERATIVO]`, `[↺ REINICIAR]`), telemetria de passada/comparação, migração para `BoxRole` (`pair`, `sorted`, `default`) e legenda semântica;
  - `GameScreen` do Bubble Sort harmonizado com subcabeçalho de telemetria e invariante, banner relacional concreto ($A[j] > A[j+1] \rightarrow \text{VERDADEIRO/FALSO}$), barra gráfica contínua de progresso (0..100%), reposicionamento de `InstructionPanel` antes da botoeira, migração para `BoxRole` e legenda semântica;
  - `CampaignCompleteScreen` refatorado para arquitetura data-driven discriminada (`campaignCompleteConfig.ts`), eliminando duplicações e bifurcações `if` no JSX, e `SelectionCampaignCompleteScreen` convertido em wrapper transparente;
  - `ResultScreen` auditado com cabeçalho de protocolo padronizado (`PROTOCOLO BUBBLE — FASE X CONCLUÍDA`), semântica `role="sorted"`, rótulo discriminado de transferências/trocas e notas pedagógicas explícitas para Bubble e Selection;
  - Preservação total de todos os testes anteriores e adição de testes para configuração data-driven (271 testes passando, 0 falhas);
- **P2.1-G-C Concluído (Hub Educacional Simétrico de Protocolos na Home):**
  - Transformação da `HomeScreen` em um Hub de Aprendizagem de Protocolos, eliminando a centralidade isolada do CTA `INICIAR TURNO` em favor da escolha curricular;
  - Criação do catálogo puramente desacoplado `src/screens/protocolCatalog.ts` com metadados de produto para Bubble Sort, Selection Sort e Insertion Sort (placeholder curricular com status EM BREVE);
  - Criação do componente simétrico e reutilizável `src/screens/ProtocolCard.tsx` com badges de status, metáfora diegética, bloco "O que você vai praticar", status factual de fases e tutoriais a partir do Schema v3, e botões padronizados de ação;
  - Acesso ao Modo Desafio (Early Exit) integrado simetricamente dentro do card do Bubble Sort condicionado ao desbloqueio factual;
  - Atalhos voluntários para Tutorial de revisão sem avanço automático de campanha;
  - Controles de Modo Demonstração preparados em todos os cards;
  - Adição de 15 novos testes unitários automatizados em `protocolCatalog.test.ts` (totalizando 286 testes, 100% passando);
- **P2.1-G-D Concluído (Institucionalização do Modo Demonstração Educacional):**
  - Criação da camada pura de demonstração `src/game/demonstration/` consumindo as engines reais (`BubbleSortEngine` e `SelectionSortEngine`) de forma autônoma sobre vetores curados fixos (`[5, 2, 4, 1]` para Bubble e `[4, 1, 3]` para Selection);
  - Garantia estrita de não-mutabilidade e volatilidade: modo observacional puro sem pontuação, sem erros, sem dicas e com zero gravações no `localStorage` ou Schema v3;
  - Telas de Replay (`ReplayScreen` e `SelectionReplayScreen`) adaptadas para suportar `mode="demonstration"`, com cabeçalho canônico (`MODO DEMONSTRAÇÃO // EXECUÇÃO CANÔNICA`), seletor de velocidade em tempo real (`0.5x`, `1x`, `2x`) e CTA opcional `▶ INICIAR TREINAMENTO`;
  - Criação do componente unificado `src/screens/DemonstrationScreen.tsx`;
  - Ativação dos botões `[ 👁 DEMONSTRAÇÃO ]` nos cards Bubble e Selection da Home e nos briefings canônicos de Bubble e Selection (`[ 👁 VER DEMONSTRAÇÃO ]`);
  - Navegação bidirecional contextual preservando origem (`Home` $\rightarrow$ `Demo` $\rightarrow$ `Home` e `Briefing` $\rightarrow$ `Demo` $\rightarrow$ `Briefing`);
  - Cobertura de 17 testes automatizados em `demonstration.test.ts` e catálogo testado, totalizando 303 testes 100% aprovados (ADR 0017).

> **Marco P2.1-G Concluído e Implementado com Sucesso.**

### Próxima Tarefa Mais Importante
1. **Módulo Insertion Sort: Mecânica de Desvio e Encaixe de Cargas (P2.2):** Engine pura, FSM de elevação e deslocamento regressivo de elementos maiores.
...
timeline
    ...
    section P2 : Médio Prazo
        Selection Sort : Mecânica de Scanner de Mínimo (IMPLEMENTADO - P2.1)
        Padronização Educacional : Charter, Telas e Glossário (IMPLEMENTADO - P2.1-G-A)
        Harmonização de Telas : Tutorial, Gameplay e Conclusão (P2.1-G-B)
        Hub de Protocolos : Cards simétricos na Home (P2.1-G-C)
        Modo Demonstração : Showcase reaproveitando Replay (P2.1-G-D)
        Insertion Sort : Mecânica de Desvio e Encaixe (P2.2)
